...
function OrderList() {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get('https://api.namtech.me/api/orders');
  ...
  const handleSetCompleted = async (orderId) => {
    try {
      await axios.put(`https://api.namtech.me/api/orders/${orderId}/complete`);
      fetchOrders();
  ...
  const handleSetPending = async (orderId) => {
    try {
      await axios.put(`https://api.namtech.me/api/orders/${orderId}/pending`);
      fetchOrders();
      Swal.fire('Thành công', 'Đã cập nhật trạng thái đơn hàng', 'success');
    } catch (error) {
      Swal.fire('Lỗi', 'Không thể cập nhật trạng thái đơn hàng', 'error');
  ...
  const handleUpdateOrder = async (updatedData) => {
    try {
      await axios.put(`https://api.namtech.me/api/orders/${selectedOrder.id}`, updatedData);
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`https://api.namtech.me/api/orders/${orderId}`);
        fetchOrders();
