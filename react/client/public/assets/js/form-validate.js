	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#Foodee-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#Foodee-contact').find('input,textarea').attr({value:''});
				jQuery('#Foodee-contact').css({opacity:1});
				jQuery('#Foodee-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#Foodee-contact').animate({opacity:0.3});
		jQuery('#Foodee-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#Foodee-contact').find('input,textarea').css('border','none');
